...
import ProfiloIscritto from './ProfiloIscritto';
        ...
        if (idAtt !== "all" && idAtt !== "") {
          ...
          if (data.iscritti.length > 0) {
            const allFields = Object.keys(data.iscritti[0]).filter(
              (key) => key !== '_id' && key !== 'luogo_n' && key !== 'indirizzo' && key !== 'cap' && key !== 'genitore' && key !== 'genitore' && key!== 'gruppo'  && key != "cap" && key != "data_iscrizione" && key != "turni" && key != "citta"  && key != "nazionalita" && key != "autorizzato_uscita"
            );
            setFields([...allFields]);
          }
        } else {
  ...
  return (
    <>
      ...
      { showProfile ? ( 
        <>
          <div className='w-container'>
            <ProfiloIscritto defaultData={selectedUser} onClose={()=>{setShowProfile(false); setSelectedUser(null);}}></ProfiloIscritto>
          </div>
        </>
      ) : (
          <>
                  ...
                  <tbody>
                    {paginatedUsers.map((user) => (
                      <tr key={user._id} className="border-t">
                        <td className="cursor-pointer text-xl"><View onClick={()=>{setSelectedUser(user); setShowProfile(true)}}></View></td>
                        {fields.map((field) => (
